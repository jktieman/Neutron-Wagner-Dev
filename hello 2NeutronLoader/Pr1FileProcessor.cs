...
namespace NeutronLoader
{
    public class Pr1FileProcessor
    {
        ...
        public Pr1FileProcessor(List<FileInfo> files, NeutronLicense neutronLicense, DynamicLogger logger, IJsonData jsonData)
        {
                ...
                switch (lineType)
                {
                    ...
                    case "2":
                        ...
                        else
                        {
                            ...
                            var stationNumber = itemDef.Station.StationNumber;
        ...
        private ItemDefinition GetItemDefinition(string partNum, string description, string oc = "C")
        {
            ...
            if (oc == "O")  //force to Off Carousel Station 8 at Dexter
            {
                ...
                try
                {
                    ...
                    item = repoItemDefinition.FindBy(r => r.Item == partNum.Trim()).FirstOrDefault();
                    if (item == null)
                    {
            ...
            else
            {
                try
                {
                    item = repoItemDefinition.FindBy(r => r.Item == partNum).FirstOrDefault();
                    if (item == null)
                    {
        ...
        private ItemDefinition GetItemDefinition(string partNum, string description)
        {
            ...
            ItemDefinition item = null;
            {
                try
                {
                    item = repoItemDefinition.FindBy(r => r.Item == sku).FirstOrDefault() ?? new ItemDefinitionProcessor(_jsonData).GetOrCreate(sku, des);
                }
                catch (Exception ex)
                {
