...
namespace NeutronLoader
{
    public class ItemDefinitionProcessor
    {
        ...
        public ItemDefinition GetOrCreate(HostOrder hostOrder)
        {
            ...
            {
                _logger.LogDetailAsync($"Error Finding Item Definition - {hostOrder.PartNum}.  {ex.Message}").SafeFireAndForget();

            }
            if (itemDef != null)
            {
                _logger.LogDetailAsync($"New Item Definition: Item:{itemDef.Item} Description:{itemDef.Description} Area:{itemDef.AreaId}").SafeFireAndForget();
            }
            return itemDef;
        }
        ...
        public ItemDefinition GetOrCreate(string partNum, string description)
        {
            ...
            {
                _logger.LogDetailAsync($"Error Finding Item Definition - {partNum}.  {ex.Message}").SafeFireAndForget();
        ...
        private ItemDefinition CreateNewItemDefinition(HostOrder hostOrder)
        {
            ...
            {
                _logger.LogDetailAsync($"Unable to create New Item Definition. {ex.Message}").SafeFireAndForget();
            }
        ...
        private ItemDefinition CreateNewItemDefinition(string partNum, string partDesc, int areaId)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogDetailAsync(msg: "No Default Item Definition.  Create Item Definition Failed.").SafeFireAndForget();
        ...
        public async Task UpdateDescriptionAsync(ItemDefinition itemDefinition, string description)
        {
            var itemDef = _repoItemDefinition.FindByKey(itemDefinition.Id);
            itemDef.Description = description;
               await _repoItemDefinition.UpdateAsync(itemDefinition);
        }
    }
}
