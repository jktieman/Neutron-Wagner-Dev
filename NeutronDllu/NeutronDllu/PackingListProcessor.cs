...
namespace NeutronDllu
{
    public class PackingListProcessor
    {
        ...
        public void PrintPackingListDocument(IList<PackingList> recs, DocumentPrinterPreferences printer, bool printPreview = false)
        {
        ...
        private void DrawBody(Graphics g, int yValue)
        {
            ...
            for (var i = 0; (i < NumRowsPerPage) && ((i + _printedRows) < _transferRecs.Count); i++)
            {
                ...
                g.DrawString(rec.Shipped, _bodyFont, Brushes.Black, rect, format);
