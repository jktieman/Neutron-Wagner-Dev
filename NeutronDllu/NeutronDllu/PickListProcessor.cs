...
namespace NeutronDllu
{
    public class PickListProcessor
    {
        ...
        public void PrintPickListDocument(IList<PickList> recs, DocumentPrinterPreferences printer, bool printPreview = false)
        {
            _transferRecs = recs;
            if (recs.Count > 0)
            {
        ...
        private void DrawBody(Graphics g, int yValue)
        {
            ...
            for (var i = 0; (i < NumRowsPerPage) && ((i + _printedRows) < _transferRecs.Count); i++)
            {
                ...
                g.DrawString(rec.Area, _bodyFont, Brushes.Black, rect, format);
                ...
                g.DrawString(rec.Slot, _bodyFont, Brushes.Black, rect, format);
