...
{
    public class ProductivitySummaryProcessor
    {
        ...
        private IList<ProductivitySummary> _transferRecs = new List<ProductivitySummary>();
        private PrintDocument _printDoc;

        public void PrintProductivitySummaryDocument(IList<ProductivitySummary> recs, DocumentPrinterPreferences printer, bool printPreview = false)
        {
            _transferRecs = recs;
            try
            {
        ...
        {
            var xValue = 0F;
            var measureString = "Productivity Summary";
        ...
        private void DrawBody(Graphics g, ref int yValue)
        {
            for (var i = 0; (i < NumRowsPerPage) && ((i + _printedRows) < _transferRecs.Count); i++)
            {
                ...
                g.DrawString(rec.Lines.ToString(), _bodyFont, Brushes.Black, rect, _formatRight);
                xValue += _columnWidths[3] + HeaderPadding;
                //SIZECODE
                rect = new RectangleF(xValue, yValue, _columnWidths[4], RowHeight);

                g.DrawString(rec.Pieces.ToString(), _bodyFont, Brushes.Black, rect, _formatRight);
                xValue += _columnWidths[4] + HeaderPadding;
                //VELOCITYCODE
                rect = new RectangleF(xValue, yValue, _columnWidths[5], RowHeight);

                g.DrawString(rec.Orders.ToString(), _bodyFont, Brushes.Black, rect, _formatRight);
                xValue += _columnWidths[5] + HeaderPadding;
                //Area
                rect = new RectangleF(xValue, yValue, _columnWidths[6], RowHeight);

                g.DrawString(rec.AreaId.ToString(), _bodyFont, Brushes.Black, rect, _formatRight);
