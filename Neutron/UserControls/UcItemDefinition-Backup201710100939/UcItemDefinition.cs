...
using MetroUIDemo.Data;
using MetroFramework.Components;
using MetroUIDemo.Models;
using Equin.ApplicationFramework;
using System.Data.Entity;
using MetroUIDemo.Repositories;

namespace MetroUIDemo.UserControls
{
    public partial class UcItemDefinition : MetroUserControl
    {
        ...
        //private GenericRepository<ItemDefinition> repo = new GenericRepository<ItemDefinition>(new NeutronEntities());
        //private BindingListView<ItemDefinition> itemDefinitionsBindingSource;
        private NeutronEntities db = new NeutronEntities();
        ...
        private ItemDefinition UpdateRecord()
        {
            var rec = new ItemDefinition()
            {
                ...
                UnitOfIssue = mtbUnitOfIssue.Text,
                LocationMax = int.Parse(mtbLocationMax.Text),
                LocationMin = int.Parse(mtbLocationMin.Text),
                SizeCode = int.Parse(mtbSizeCode.Text),
                VelocityCode = int.Parse(mtbVelocityCode.Text),
                StorageType = 2, // int.Parse(mtbStorageType.Text),
                ...
                HeightCode = int.Parse(mtbHeightCode.Text),
                Station = int.Parse(mtbStation.Text)
            };
            return rec;
        }
        ...
        private ItemDefinition CreateNewRecord()
        {
            var rec = new ItemDefinition()
            {
                ...
                UnitOfIssue = mtbUnitOfIssueNew.Text,
                LocationMax = int.Parse(mtbLocationMaxNew.Text),
                LocationMin = int.Parse(mtbLocationMinNew.Text),
                SizeCode = int.Parse(mtbSizeCodeNew.Text),
                VelocityCode = int.Parse(mtbVelocityCodeNew.Text),
                StorageType = 2, // int.Parse(mtbStorageType.Text),
                ...
                HeightCode = int.Parse(mtbHeightCodeNew.Text),
                Station = int.Parse(mtbStationNew.Text)
            };
            return rec;
        }
        ...
        {
            mpGrid.Visible = false;
            mpDetailNew.Visible = false;
