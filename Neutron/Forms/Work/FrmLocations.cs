        ...
        private void MoveDevice(int deviceNumber, int trayNumber, int level = 0, int partition = 0, int quantity = 0, string display = "")
        {
            if (_neutronVariables.ShuttleEnabled)
            {
                ...
                if (hardwareDevice != null)
                {
                    ...
                    {
                        MessageBox.Show($"Hardware Device is not enabled.{hardwareDevice.Name}");
                    }
                }
                else
                {
                ...
                if (IntegerValidator(IntegerExtensions.ParseInt(TextBoxNewLoc3.Text)))
                {
                    ...
                    if (IntegerValidator(IntegerExtensions.ParseInt(TextBoxNewLoc4.Text)))
                    {
                        ...
                        if (IntegerValidator(IntegerExtensions.ParseInt(TextBoxNewLoc5.Text)))
                        {
                            ...
                            {
                                MessageBox.Show("Location already exists.", string.Empty,MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
                        else
                        {
        ...
        private void SetCulture(string lang)
        {
            try
            {
               ...
               resourceDir: languageDirectory, usingResourceSet: null);
                ...
                _resourceManager.GetString("Message11");
                _resourceManager.GetString("Message12");
                _resourceManager.GetString("Message13");
                _resourceManager.GetString("Message14");
                _resourceManager.GetString("Message15");
                _resourceManager.GetString("Message16");
                _resourceManager.GetString("Message17");
                _resourceManager.GetString("Message18");
                _resourceManager.GetString("Message19");
            }
            catch (Exception ex)
            {
