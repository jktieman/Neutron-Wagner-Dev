...
namespace Neutron.Forms
{
    public partial class FrmHeightCodes : MetroForm
    {
        ...
        private void FrmHeightCodes_Load(object sender, EventArgs e)
        {
            int id = RefreshData();
        ...
        {
            int idx = 0;
            List<HeightCode> recs = repoHeightCode.All().OrderBy(o => o.Sequence).ToList();
        ...
        public int IndexOf(int value)
        {
            ...
            {
                int rec = ((HeightCode) bindingSource[i]).Id;
                if (rec == value)
                {
                    itemIndex = i;
                    break;
        ...
        private void FindRecord(string s)
        {
            try
            {
                ...
                {
                    bindingSource.DataSource = repoHeightCode.All().OrderBy(o => o.Sequence).ToList();
                    GetRecordCount();
                }
                else
                {
                    bindingSource.DataSource = repoHeightCode.All().OrderBy(o => o.Sequence).Where(d => d.Name.Contains(s)).ToList();
                    GetRecordCount();
                }
            }
            catch (Exception ex)
        ...
        {
            int id = ((HeightCode) bindingSource.Current).Id;
            var rec = new HeightCode
            {
                Id = id,
            ...
            repoHeightCode.Update(rec);
            RefreshData(rec.Id);
            tabControl1.SelectedTab = tabPage1;
        }

        private bool ValidateFields(HeightCode rec)
        {
       ...
       private bool IntegerValidator(int input)
        ...
        private bool IsDuplicate(HeightCode recIn)
        {
            HeightCode rec = repoHeightCode.FindBy(f => f.Name == recIn.Name).FirstOrDefault();
            if (rec != null)
            {
