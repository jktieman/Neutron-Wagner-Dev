...
namespace Neutron.Forms
{
    public partial class FrmVelocityCodes : MetroForm
    {
        ...
        private void FrmVelocityCodes_Load(object sender, EventArgs e)
        {
            int id = RefreshData();
        ...
        {
            int idx = 0;
            List<VelocityCode> recs = repoVelocityCode.All().OrderBy(o => o.Sequence).ToList();
        ...
        public int IndexOf(int value)
        {
            ...
            {
                int rec = ((VelocityCode) bindingSource[i]).Id;
                if (rec == value)
                {
                    itemIndex = i;
                    break;
        ...
        private void FindRecord(string s)
        {
            try
            {
                ...
                {
                    bindingSource.DataSource = repoVelocityCode.All().OrderBy(o => o.Sequence).ToList();
                    GetRecordCount();
                }
                else
                {
                    bindingSource.DataSource = repoVelocityCode.All().OrderBy(o => o.Sequence).Where(d => d.Name.Contains(s)).ToList();
                    GetRecordCount();
                }
            }
            catch (Exception ex)
        ...
        {
            int id = ((VelocityCode) bindingSource.Current).Id;
            var rec = new VelocityCode
            {
                Id = id,
            ...
            repoVelocityCode.Update(rec);
            RefreshData(rec.Id);
            tabControl1.SelectedTab = tabPage1;
        }

        private bool ValidateFields(VelocityCode rec)
        {
        ...
        private bool IsDuplicate(VelocityCode recIn)
        {
            VelocityCode rec = repoVelocityCode.FindBy(f => f.Name == recIn.Name).FirstOrDefault();
            if (rec != null)
            {
        ...
        private void SetupNewForm()
        {
            ...
            LabelFormTitle.Text = "Velocity Codes";
